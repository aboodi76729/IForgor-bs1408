...
void IForgor::PauseUIManager::CreateUIElements()
{
    ...
    if (_colorScheme == nullptr) {
        ...
        if (installers.size() > 0) {
            // In BS 1.40+, sceneSetupData access via getter or direct field
            _colorScheme = installers[0]->_sceneSetupData->colorScheme;
        }
    }
    ...
    groupA->Initialize();
    groupB->Initialize();
    groupA->SetNoteColor(_colorScheme->saberAColor);
    groupB->SetNoteColor(_colorScheme->saberBColor);
    groupA->SetSaberColor(_colorScheme->saberAColor);
    groupB->SetSaberColor(_colorScheme->saberBColor);
...
#endif
    auto leftColor = _colorScheme->saberAColor;
    auto rightColor = _colorScheme->saberBColor;
#ifdef CHROMA
